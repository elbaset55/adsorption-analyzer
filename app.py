...
with tab_rep:
    ...
    if st.button(t("generate_btn"),use_container_width=True):
        try:
            ...
            CG="34D399"; CGL="DCFCE7"; CR="F87171"; CRL="FEE2E2"
            CY="D97706"; CYL="FEF9C3"; CBL="2563EB"; CGR="6B7280"
            ...
            def _ac(h="center",v="center",wrap=True):
                return Alignment(horizontal=h,vertical=v,wrap_text=wrap)

            def _hrow(ws,row,ncols,bg=CP,fg=CW,sz=11,ht=28):
                ws.row_dimensions[row].height=ht
                for c in range(1,ncols+1):
                    ce=ws.cell(row=row,column=c)
                    ce.font=_f(sz,True,fg); ce.fill=_fill(bg)
                    ce.alignment=_ac(); ce.border=_brd

            def _title(ws,text,ncols,bg=CB,sz=14,ht=38):
                ws.merge_cells(f"A1:{get_column_letter(ncols)}1")
                c=ws["A1"]; c.value=text
                c.font=_f(sz,True,CW); c.fill=_fill(bg)
                c.alignment=_ac(); ws.row_dimensions[1].height=ht
            ...
            def _r2color(v):
                return C_OK if v>=0.99 else (CY if v>=0.95 else C_BAD)

            # favorability logic (shared)
            def _iso_fav(nm,params):
                if nm=="Langmuir":
                    RL=params.get("RL",0.5)
                    if RL<=0:   return f"RL={RL:.4f} — Irreversible","7C3AED","Irreversible adsorption; adsorbate strongly bound to surface."
                    elif RL<1:  return f"RL={RL:.4f} — Favorable \u2714",C_OK,f"Favorable monolayer adsorption (0<RL<1). High affinity between adsorbate and adsorbent."
                    elif RL==1: return f"RL={RL:.4f} — Linear",CY,"Linear isotherm; adsorption directly proportional to concentration."
                    else:       return f"RL={RL:.4f} — Unfavorable \u2718",C_BAD,f"Unfavorable conditions (RL>1). Low affinity; consider modifying system conditions."
                elif nm=="Freundlich":
                    inv_n=params.get("1/n",1.0)
                    if inv_n<1:   return f"1/n={inv_n:.4f} — Favorable \u2714",C_OK,f"Favorable heterogeneous adsorption (1/n<1). Multilayer uptake with energetic site distribution."
                    elif inv_n>1: return f"1/n={inv_n:.4f} — Unfavorable \u2718",C_BAD,f"Unfavorable conditions (1/n>1). Indicates cooperative or anti-Langmuir behavior."
                    else:         return f"1/n={inv_n:.4f} — Linear",CY,"Linear Freundlich isotherm; homogeneous surface energy distribution."
                elif nm=="D-R":
                    E=params.get("E (kJ/mol)",0); at=params.get("Type","")
                    if "Physi" in at: return f"E={E:.2f} kJ/mol — Physisorption",CBL,f"E < 8 kJ/mol: Physical forces (van der Waals) dominant. Reversible, low-energy adsorption."
                    elif "Ion" in at: return f"E={E:.2f} kJ/mol — Ion Exchange",CY,f"8 \u2264 E < 16 kJ/mol: Ion exchange mechanism. Intermediate binding energy."
                    else:             return f"E={E:.2f} kJ/mol — Chemisorption",C_BAD,f"E \u2265 16 kJ/mol: Chemical bond formation. Strong, potentially irreversible adsorption."
                elif nm=="Sips":
                    ns=params.get("ns",1.0)
                    if abs(ns-1)<0.15: return f"ns={ns:.4f} — \u2248Langmuir",CY,f"ns\u22481: Nearly homogeneous surface (approaches Langmuir model). ns={ns:.4f}."
                    else:              return f"ns={ns:.4f} — Heterogeneous",C_OK,f"ns\u22601: Heterogeneous surface with distributed binding energies. ns={ns:.4f}."
                elif nm=="Temkin":
                    bT=params.get("bT (J/mol)",0)
                    lbl="Physisorption" if bT<40000 else "Chemisorption"
                    clr=CBL if bT<40000 else C_BAD
                    return f"bT={bT:.0f} J/mol — {lbl}",clr,f"Heat of adsorption bT={bT:.0f} J/mol. {'<40 kJ/mol: physical adsorption dominant.' if bT<40000 else '\u226540 kJ/mol: chemical interactions dominant.'}"
                elif nm=="BET":
                    C_b=params.get("C_BET",0)
                    return f"C_BET={C_b:.2f}",CP,f"BET constant C={C_b:.2f}. Multilayer adsorption model. C>>1 indicates strong adsorbate–surface interaction."
                return "—","1F2937","—"
            ...
            # SHEET 1 — COVER
            # ══════════════════════════════════════════════════════════════
            wc=wb.create_sheet("Cover"); _set_cols(wc,[32,46,22])
            _title(wc,"AdsorpLab Pro — Scientific Adsorption Analysis Report",3,CB,16,42)
            wc.merge_cells("A2:C2"); s=wc["A2"]
            s.value="by Diana Raie, Abdelbaset A. A. Diyab"
            s.font=_f(11,False,CW,italic=True); s.fill=_fill(CP); s.alignment=_ac()
            wc.row_dimensions[2].height=22

            meta=[("Experiment",exp_nm),("Researcher",rsr or "—"),
                  ("Notes",nts or "—"),("Generated By","AdsorpLab Pro v2.0"),
                  ("Date",pd.Timestamp.now().strftime("%Y-%m-%d %H:%M"))]
            for i,(k,v) in enumerate(meta):
                r=i+4; wc.row_dimensions[r].height=21
                a=wc.cell(r,1,k); a.font=_f(10,True,CP); a.fill=_fill(CL); a.alignment=_ac("left"); a.border=_brd
                b=wc.cell(r,2,v); b.font=_f(10,color="1F2937"); b.fill=_fill(CW); b.alignment=_ac("left"); b.border=_brd
                wc.cell(r,3,"").fill=_fill(CW); wc.cell(r,3).border=_brd

            r=10; wc.row_dimensions[r].height=26; wc.merge_cells(f"A{r}:C{r}")
            s=wc[f"A{r}"]; s.value="QUICK SUMMARY"; s.font=_f(12,True,CW); s.fill=_fill(CP); s.alignment=_ac()

            smry=[]
            if "iso_results" in st.session_state:
                bi=st.session_state.get("iso_best","—"); bv=st.session_state["iso_results"].get(bi,{}).get("r2",0)
                smry.append(("Best Isotherm Model",bi,f"R\u00b2 = {bv:.4f}","059669" if bv>=0.99 else CY))
            if "kin_results" in st.session_state:
                bk=st.session_state.get("kin_best","—"); bkv=st.session_state["kin_results"].get(bk,{}).get("r2",0)
                smry.append(("Best Kinetic Model",bk,f"R\u00b2 = {bkv:.4f}","059669" if bkv>=0.99 else CY))
            if "thermo_results" in st.session_state:
                tr2=st.session_state["thermo_results"]; dHs=tr2["delta_H"]
                nat="Exothermic" if dHs<0 else "Endothermic"
                aty="Physisorption" if abs(dHs)<20 else ("Mixed" if abs(dHs)<40 else "Chemisorption")
                smry.append(("Thermodynamic Nature",f"{nat} — {aty}",f"\u0394H\u00b0 = {dHs:.3f} kJ/mol","059669" if dHs<0 else C_BAD))

            for i,(label,val,stat,vc) in enumerate(smry):
                r=11+i; wc.row_dimensions[r].height=21
                a=wc.cell(r,1,label); a.font=_f(10,True); a.fill=_fill(CA); a.alignment=_ac("left"); a.border=_brd
                b=wc.cell(r,2,val); b.font=_f(10,True,vc); b.fill=_fill(CW); b.alignment=_ac("left"); b.border=_brd
                c2=wc.cell(r,3,stat); c2.font=_f(9,color=CGR); c2.fill=_fill(CW); c2.alignment=_ac(); c2.border=_brd
            ...
            if "iso_results" in st.session_state:
                ...
                wi=wb.create_sheet("Isotherm Analysis"); _set_cols(wi,[20,10,10,10,30,42])
                _title(wi,"ISOTHERM ANALYSIS — MODEL COMPARISON & PARAMETERS",6,CB,14,36)
                hdrs=["Model","R\u00b2","RMSE","\u03c7\u00b2","Favorability Indicator","Scientific Interpretation"]
                _hrow(wi,2,6); 
                for ci,h in enumerate(hdrs):
                    ce=wi.cell(2,ci+1,h); ce.font=_f(10,True,CW); ce.fill=_fill(CP); ce.alignment=_ac(); ce.border=_brd
                ...
                for ri,(nm,res) in enumerate(srt):
                    ...
                    for ci,(val,clr,bold,algn) in enumerate([
                        (("★ " if ib else "")+nm, CP if ib else "1F2937", ib, "left"),
                        (round(r2v,4),  _r2color(r2v), True,  "center"),
                        (round(res["rmse"],4),"374151",False,"center"),
                        (round(res["chi2"],4),"374151",False,"center"),
                        (fl, fc, True, "left"),
                        (fi, "374151", False, "left"),
                    ]):
                        ce=wi.cell(row,ci+1,val); ce.font=_f(10 if ci>0 else 10,bold,clr)
                        ce.fill=_fill(bg); ce.border=_brd; ce.alignment=_ac(algn)

                # Parameters sub-table
                pr=len(srt)+4; wi.row_dimensions[pr].height=26
                wi.merge_cells(f"A{pr}:F{pr}"); s=wi[f"A{pr}"]
                s.value="MODEL PARAMETERS — DETAILED"; s.font=_f(11,True,CW)
                s.fill=_fill(CP); s.alignment=_ac()
                pr+=1; _hrow(wi,pr,4)
                for ci,h in enumerate(["Model","Parameter","Value","Unit / Notes"]):
                    ce=wi.cell(pr,ci+1,h); ce.font=_f(10,True,CW); ce.fill=_fill(CP); ce.alignment=_ac(); ce.border=_brd

                _punits={"qₘₐₓ (mg/g)":"mg/g","KL (L/mg)":"L/mg","RL":"dimensionless  |  0<RL<1: favorable",
                         "KF":"(mg/g)(L/mg)^(1/n)","1/n":"dimensionless  |  <1: favorable","n":"dimensionless",
                         "AT (L/g)":"L/g","B (mg/g)":"J/mol","bT (J/mol)":"J/mol  |  <40 000: physisorption",
                         "qₘ (mg/g)":"mg/g","\u03b2 (mol\u00b2/kJ\u00b2)":"mol\u00b2/kJ\u00b2",
                         "E (kJ/mol)":"kJ/mol  |  <8: physi  |  8\u201316: ion exch  |  >16: chemi",
                         "Type":"Adsorption mechanism","Ks":"(L/mg)^ns","ns":"heterogeneity  |  ns=1 \u2192 Langmuir",
                         "Cs (mg/L)":"mg/L","C_BET":"dimensionless BET constant"}
                pr+=1
                for ri2,(nm,res) in enumerate(srt):
                    ...
                    for pk,pv in res["params"].items():
                        ...
                        for ci2,(val,bold,clr,algn) in enumerate([
                            (nm,True,CP,"left"),(pk,True,"1F2937","left"),
                            (f"{pv:.6f}" if isinstance(pv,float) else str(pv),False,"1D4ED8","center"),
                            (_punits.get(pk,""),False,CGR,"left")]):
            ...
            if "kin_results" in st.session_state:
                ...
                wk=wb.create_sheet("Kinetics Analysis"); _set_cols(wk,[22,10,10,10,30,44])
                _title(wk,"KINETICS ANALYSIS — MODEL COMPARISON & PARAMETERS",6,CB,14,36)
                for ci,h in enumerate(["Model","R\u00b2","RMSE","\u03c7\u00b2","Mechanism","Scientific Interpretation"]):
                    ce=wk.cell(2,ci+1,h); ce.font=_f(10,True,CW); ce.fill=_fill(CP); ce.alignment=_ac(); ce.border=_brd
                wk.row_dimensions[2].height=28

                _mechs={
                    "PFO":  ("Pseudo-First-Order — Physical Adsorption",
                             "Rate \u221d (qe\u2212qt). If R\u00b2\u22650.99: film diffusion likely rate-limiting. Indicates physisorption or reversible binding."),
                    "PSO":  ("Pseudo-Second-Order — Chemisorption",
                             "Rate \u221d (qe\u2212qt)\u00b2. Best fit suggests chemical bond formation. PSO > PFO in R\u00b2 \u2192 chemisorption dominant."),
                    "Elovich": ("Elovich — Heterogeneous Chemisorption",
                                "\u03b1: initial sorption rate (mg/g\u00b7min); \u03b2: desorption constant (g/mg). Applies to activated, energetically heterogeneous surfaces."),
                    "Weber-Morris": ("Weber-Morris — Intraparticle Diffusion",
                                     "kid: diffusion rate constant. C\u22480 \u2192 IPD rate-limiting. C>0 \u2192 combined film + intraparticle diffusion (multi-step mechanism).")
                }
                ...
                s.value="KINETIC PARAMETERS — DETAILED"; s.font=_f(11,True,CW); s.fill=_fill(CP); s.alignment=_ac()
                pr+=1
                for ci,h in enumerate(["Model","Parameter","Value","Unit / Notes"]):
                    ce=wk.cell(pr,ci+1,h); ce.font=_f(10,True,CW); ce.fill=_fill(CP); ce.alignment=_ac(); ce.border=_brd
                wk.row_dimensions[pr].height=22; pr+=1
                _ku={"k1 (1/min)":"1/min","qe (mg/g)":"mg/g","k2 (g/mg\u00b7min)":"g/mg\u00b7min",
                     "\u03b1 (mg/g\u00b7min)":"mg/g\u00b7min — initial sorption rate",
                     "\u03b2 (g/mg)":"g/mg — desorption constant",
                     "kid (mg/g\u00b7min^0.5)":"mg/g\u00b7min^0.5","C (mg/g)":"mg/g — boundary layer thickness"}
                for ri2,(nm,res) in enumerate(srtk):
                    ...
                    for pk,pv in res["params"].items():
                        ...
                        for ci2,(val,bold,clr,algn) in enumerate([
                            (nm,True,CP,"left"),(pk,True,"1F2937","left"),
                            (f"{pv:.6f}",False,"1D4ED8","center"),(_ku.get(pk,""),False,CGR,"left")]):
            ...
            if "thermo_results" in st.session_state:
                tr=st.session_state["thermo_results"]
                wt=wb.create_sheet("Thermodynamics"); _set_cols(wt,[22,16,32,28,10])
                _title(wt,"THERMODYNAMIC ANALYSIS — Van\u2019t Hoff",5,CB,14,36)
                for ci,h in enumerate(["Parameter","Value","Description","Interpretation",""]):
                    ce=wt.cell(2,ci+1,h); ce.font=_f(10,True,CW); ce.fill=_fill(CP); ce.alignment=_ac(); ce.border=_brd
                wt.row_dimensions[2].height=26

                dHv=tr["delta_H"]; dSv=tr["delta_S"]; r2v=tr["r2"]
                aty="Physisorption" if abs(dHv)<20 else ("Mixed physi/chemisorption" if abs(dHv)<40 else "Chemisorption")
                tp=[
                    ("\u0394H\u00b0 (kJ/mol)",f"{dHv:.4f}","Enthalpy change of adsorption",
                     ("Exothermic: heat released, spontaneous tendency" if dHv<0 else "Endothermic: heat absorbed, driven by entropy"),
                     C_OK if dHv<0 else C_BAD),
                    ("\u0394S\u00b0 (J/mol\u00b7K)",f"{dSv:.4f}","Entropy change at interface",
                     ("Increased randomness at solid\u2013liquid interface (+\u0394S\u00b0)" if dSv>0 else "Decreased randomness; ordered adsorption layer"),
                     C_OK if dSv>0 else CGR),
                    ("Van\u2019t Hoff R\u00b2",f"{r2v:.4f}","Linearity of ln(Kc) vs 1/T",
                     ("Excellent linear fit \u2014 thermodynamic data reliable" if r2v>=0.99 else ("Good fit" if r2v>=0.95 else "Moderate fit \u2014 check data consistency")),
                     C_OK if r2v>=0.99 else (CY if r2v>=0.95 else C_BAD)),
                    ("Adsorption Type",aty,"|ΔH°|: <20→Physi | 20\u201340→Mixed | >40→Chemi",
                     f"|ΔH°| = {abs(dHv):.1f} kJ/mol",CP),
                ]
                for ri,(p,v,d,interp,vc) in enumerate(tp):
                    row=ri+3; bg=CA if ri%2 else CW; wt.row_dimensions[row].height=22
                    for ci2,(val,bold,clr,algn) in enumerate([
                        (p,True,"1F2937","left"),(v,True,"1D4ED8","center"),
                        (d,False,CGR,"left"),(interp,True,vc,"left"),("",False,CGR,"center")]):
                        ce=wt.cell(row,ci2+1,val); ce.font=_f(10,bold,clr)
                        ce.fill=_fill(bg); ce.border=_brd; ce.alignment=_ac(algn)

                # ΔG° table
                pr=8; wt.row_dimensions[pr].height=26; wt.merge_cells(f"A{pr}:E{pr}")
                s=wt[f"A{pr}"]; s.value="GIBBS FREE ENERGY vs TEMPERATURE (\u0394G\u00b0)"; s.font=_f(11,True,CW)
                s.fill=_fill(CP); s.alignment=_ac(); pr+=1
                dg=tr["df"]
                for ci,col in enumerate(dg.columns):
                    ce=wt.cell(pr,ci+1,col); ce.font=_f(10,True,CW); ce.fill=_fill(CP); ce.alignment=_ac(); ce.border=_brd
                ...
                for ri2,row_d in dg.iterrows():
                    ...
                    for ci2,val in enumerate(row_d):
                        ...
                        ce.font=_f(9,is_dg,C_OK if (is_dg and isinstance(vf,(int,float)) and vf<0) else (C_BAD if (is_dg and isinstance(vf,(int,float)) and vf>=0) else "374151"))
                        ce.fill=_fill(bg); ce.border=_brd; ce.alignment=_ac()
                    pr+=1

            # ══════════════════════════════════════════════════════════════
            # SHEET 5 & 6 — RAW DATA
            # ══════════════════════════════════════════════════════════════
            for sheet_nm,ss_key in [("Isotherm Raw Data","iso_df"),("Kinetics Raw Data","kin_df")]:
                if ss_key in st.session_state:
                    ...
                    for ci,col in enumerate(df_src.columns):
                        ...
                        ce.fill=_fill(CP); ce.alignment=_ac(); ce.border=_brd
